package com.fisherevans.scs;

/**
 * Created by h13730 on 11/2/2015.
 */
public class Constants {
  public static final String CONFIG_FILE = "plugins/SudoCraftSuite.yml";


  public static final String CONFIG_KEY_HOMES = "homes";
  public static final String CONFIG_KEY_HOMES_DEFAULT_NAME = "default-home-name";
  public static final String DEFAULT_HOMES_NAME = "Default";

  public static final String CONFIG_KEY_BACKS = "backs";

  public static final String CONFIG_KEY_TP_TIMEOUT = "tp-timeout";
  public static final Integer DEFAULT_TP_TIMEOUT = 30;

  public static final String CONFIG_CHAT_FORMAT = "chat-format";
  public static final String DEFAULT_CHAT_FORMAT = "#gray#name#white: #message";

  public static final String CONFIG_VOTE_REQUIRED = "vote-required";
  public static final String CONFIG_VOTE_TIMEOUT = "vote-timeout";
  public static final Double DEFAULT_VOTE_REQUIRED = .8;
  public static final Integer DEFAULT_VOTE_TIMEOUT = 45;
}
